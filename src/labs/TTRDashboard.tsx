...
function PreviousSeasonRow({
  season,
  teamId,
}: {
  season: { label: string; leagueId: number | null; seasonId: number | null; divisionId: number | null };
  teamId: number;
}) {
  ...
  function toggle() {
    if (!open && !profile && !loading && season.leagueId && season.seasonId && season.divisionId) {
      setLoading(true);
      const params = new URLSearchParams({
        leagueId: String(season.leagueId),
        divisionId: String(season.divisionId),
        seasonId: String(season.seasonId),
        teamId: String(teamId),
        type: 'team-profile',
      });
      fetch(`/api/ttr?${params}`)
        .then(r => r.json())
        .then(j => { setProfile(j.data as TeamProfile); setLoading(false); })
  ...
  const ss = profile?.stats.season;
  const rec = profile?.currentRecord;
  ...
  return (
    <div style={{ borderBottom: `1px solid ${BORDER}` }}>
      ...
      >
        <span className="text-sm font-medium text-black/65">{season.label}</span>
        <span className="font-mono text-[10px] text-black/30">{open ? '▲' : '▼'}</span>
      </button>
      {open && (
        <div className="px-4 pb-4">
          {loading && <span className="font-mono text-xs text-black/35">Loading…</span>}
          {err && <span className="font-mono text-xs" style={{ color: RED }}>{err}</span>}
          {profile && (
            <div className="flex flex-wrap gap-x-6 gap-y-2 pt-1">
              {profile.currentPosition !== null && (
                <div className="flex items-baseline gap-1.5">
                  <span className="font-mono text-xl font-black" style={{ color: RED }}>#{profile.currentPosition}</span>
                  <span className="font-mono text-[10px] uppercase tracking-widest text-black/40">pos</span>
                </div>
              )}
              {rec && (
                <>
                  <div className="flex items-baseline gap-1.5">
                    <span className="font-mono text-xl font-black" style={{ color: RED }}>{rec.w}</span>
                    <span className="font-mono text-[9px] uppercase tracking-widest text-black/40 ml-0.5">W</span>
                  </div>
                  <div className="flex items-baseline gap-1.5">
                    <span className="font-mono text-xl font-black text-black/40">{rec.l}</span>
                    <span className="font-mono text-[9px] uppercase tracking-widest text-black/40 ml-0.5">L</span>
                  </div>
                  {rec.d > 0 && (
                    <div className="flex items-baseline gap-1.5">
                      <span className="font-mono text-xl font-black text-black/30">{rec.d}</span>
                      <span className="font-mono text-[9px] uppercase tracking-widest text-black/40 ml-0.5">D</span>
                    </div>
                  )}
                </>
              )}
              {ss?.avgScored != null && (
                <div className="flex items-baseline gap-1.5">
                  <span className="font-mono text-sm font-bold text-black/65">{ss.avgScored.toFixed(1)}</span>
                  <span className="font-mono text-[10px] uppercase tracking-widest text-black/35">avg scored</span>
                </div>
              )}
              {ss?.avgConceded != null && (
                <div className="flex items-baseline gap-1.5">
                  <span className="font-mono text-sm font-bold text-black/45">{ss.avgConceded.toFixed(1)}</span>
                  <span className="font-mono text-[10px] uppercase tracking-widest text-black/35">avg conceded</span>
                </div>
              )}
              {ss?.biggestWin && (
                <div className="flex items-baseline gap-1.5">
                  <span className="font-mono text-sm font-bold" style={{ color: RED }}>{ss.biggestWin}</span>
                  <span className="font-mono text-[10px] uppercase tracking-widest text-black/35">best win</span>
                </div>
              )}
              <div className="w-full mt-1 flex gap-4">
                <span className="font-mono text-[10px] text-black/30">
                  {profile.seasonMatches.filter(m => m.result).length} games played
                </span>
                {profile.pomAwards.length > 0 && (
                  <span className="font-mono text-[10px] text-black/30">
                    {profile.pomAwards.reduce((sum, p) => sum + p.awards, 0)} POM awards
                  </span>
                )}
              </div>
            </div>
          ...
          {!loading && !err && !profile && (
            <span className="font-mono text-xs text-black/30">No data available</span>
          )}
        </div>
      )}
    </div>
